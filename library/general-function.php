<?php 
	...
	function addPrefixToPhone($sPhone){ 
		$sPhoneWithPrefix = '';
		$iMobileLength = strlen(trim($sPhone));
		$sPhoneStartWith = substr($sPhone, 0,1);
		if ($iMobileLength==10 && $sPhoneStartWith!=1){
			$sPhoneWithPrefix = '1'.$sPhone;// adding country code 1 for US and Canada
		}
		else{
			$sPhoneWithPrefix = $sPhone;
		}
		return $sPhoneWithPrefix;
	}
